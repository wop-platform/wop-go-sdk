...
func decodeKeyMaterial(material string) ([]byte, error) {
	...
	if trimmed == "" {
		return nil, newError(CodeConfiguration, "密钥材料为空")
	}
	...
	if err != nil {
		return nil, newError(CodeConfiguration, "密钥 Base64 解码失败：%v", err)
	}
	return der, nil
}
...
func parseRSAPublicKey(material string) (*rsa.PublicKey, error) {
	...
	if err != nil {
		return nil, newError(CodeConfiguration, "RSA 公钥解析失败（须 X.509 SPKI，PEM 或 Base64）：%v", err)
	}
	rsaPub, ok := pub.(*rsa.PublicKey)
	if !ok {
		return nil, newError(CodeConfiguration, "密钥不是 RSA 公钥（实际 %T）", pub)
	}
	return rsaPub, nil
}
...
func parseRSAPrivateKey(material string) (*rsa.PrivateKey, error) {
	...
	if err != nil {
		return nil, newError(CodeConfiguration, "RSA 私钥解析失败（须 PKCS#8，PEM 或 Base64）：%v", err)
	}
	rsaKey, ok := key.(*rsa.PrivateKey)
	if !ok {
		return nil, newError(CodeConfiguration, "密钥不是 RSA 私钥（实际 %T）", key)
	}
	return rsaKey, nil
}
...
func parseSM2PublicKey(material string) (*ecdsa.PublicKey, error) {
	...
	if len(raw) != 65 || raw[0] != 0x04 {
		return nil, newError(CodeConfiguration, "SM2 公钥须为未压缩点 04‖X‖Y 共 65 字节（Base64），实际 %d 字节", len(raw))
	}
	pub, err := sm2.NewPublicKey(raw)
	if err != nil {
		return nil, newError(CodeConfiguration, "SM2 公钥点非法（不在 sm2p256v1 曲线上）：%v", err)
	}
	return pub, nil
}
...
func parseSM2PrivateKey(material string) (*ecdsa.PrivateKey, error) {
	...
	if len(raw) != 32 {
		return nil, newError(CodeConfiguration, "SM2 私钥须为 32 字节大端标量 d（Base64），实际 %d 字节", len(raw))
	}
	d := new(big.Int).SetBytes(raw)
	n := sm2.P256().Params().N
	if d.Sign() == 0 || d.Cmp(n) >= 0 {
		return nil, newError(CodeConfiguration, "SM2 私钥标量 d 超出 [1, n-1] 范围")
	}
...
func validateRSASize(s Suite, bits int) error {
	if bits != s.KeyBits() {
		return newError(CodeConfiguration, "RSA 密钥位数 %d 与套件 %s 要求的 %d 位不符",
